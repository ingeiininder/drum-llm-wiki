---
type: entity
book_id: MI_CURR
source: IMG_1563
unit: 10
tags: [performance_notes, odd_meter, fusion, double_bass]
last_updated: 2026-04-19
---

# Dog (Composition)

**Dog** is a high-energy fusion composition by **Conrad Schrenk**, originally recorded in 2000. The piece is characterized by its aggressive delivery, complex [[Time Signature]] fluctuations, and demanding [[Double Pedal]] technical requirements.

### Technical Overview
The track serves as a comprehensive study in modern drumming, requiring mastery over:
*   **Metric Modulation and Odd Meters**: The song utilizes a recurring cycle of 7/4 and 9/4.
*   **Orchestration**: Use of a secondary 12" [[Snare Drum]] to provide timbral contrast against the primary 14" snare.
*   **Unison Phrasing**: A highly unusual section where the drummer performs [[Unison Foot Phrasing]] with a busy guitar solo while maintaining a straight time feel with the hands.

### Sectional Analysis
1.  **Intro/Verse**: Features a 7/4 - 9/4 cycle. The drummer utilizes a 12" snare for a tighter, "pop" sound to differentiate the intro from the main groove.
2.  **Bridge**: Written in 5/8 but phrased as 10/8. This creates a "half-time" feel, lengthening the cycle and providing a rhythmic breather before the "4 on the floor" build-up.
3.  **Guitar Solo**: The technical climax of the song. The drummer must execute [[Double Pedal]] patterns that mirror the guitar's rhythmic melody in unison.
4.  **Coda**: A 16th-note run orchestrated across the [[Tom-Toms]] and snare, incorporating [[Cross-Sticking]] and finishing with a manic double-kick barrage.

### Pro-Tips
*   **Aggression**: The "pushed 1" at the end of every second bar in the intro requires significant physical weight to anchor the band.
*   **Balance**: When playing the guitar solo unison parts, focus on keeping the hands "blasé" or relaxed to ensure the time doesn't fluctuate while the feet are busy.