---
type: technique
source: IMG_0172
unit: Unit 5 - Chapter 1
tags: [orchestration, dynamics, texture, arrangement, hi-hat, ride cymbal, drum kit]
last_updated: 2023-10-27
---

# Playing Surface Switch
A "Playing Surface Switch" refers to the act of changing the primary drum or cymbal being used for time-keeping or groove articulation within a song. This technique is a fundamental aspect of drum set orchestration, used to create dynamic contrast, define different song sections, and alter the overall texture of the drum part.

Common examples include switching from:
*   [[Hi-Hat]] to [[Ride Cymbal]] (as seen in the [[Bridge (Song Section)]] within the [[Coda (Song Section)]] in this curriculum).
*   Hi-hat to [[Open Hi-Hat]].
*   Ride cymbal to [[Floor Tom]] or [[Snare Drum]] rim.

This switch often coincides with changes in [[Song Structure]] (e.g., moving from a [[Verse (Song Section)]] to a [[Chorus (Song Section)]] or entering a [[Bridge (Song Section)]]), [[New Groove]]s, or shifts in [[Harmonic Structure]].

**Pro-Tip:** Masterful drummers use playing surface switches intentionally to serve the song. Practice smooth transitions between surfaces, ensuring that the new groove starts cleanly and with the desired feel. Consider the dynamic and timbral impact of each surface and how it contributes to the emotional arc of the music.